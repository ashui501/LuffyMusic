...
from LuffyMusic.utils.inline.play import (stream_markup,
                                          telegram_markup,
                                          close_keyboard)
from LuffyMusic.utils.stream.autoclear import auto_clean
from LuffyMusic.utils.thumbnails import gen_thumb
...
async def skip(cli, message: Message, _, chat_id):
    ...
    if "live_" in queued:
        ...
        try:
            ...
            image = None
        try:
            await Luffy.skip_stream(chat_id, file_path, video=status, image=image)
        except Exception:
            return await mystic.edit_text(_["call_9"])
        ...
        try:
            await Luffy.skip_stream(chat_id, videoid, video=status)
        except Exception:
            return await message.reply_text(_["call_9"])
        button = telegram_markup(_, chat_id)
        run = await message.reply_photo(
